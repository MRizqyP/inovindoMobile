...
class PembayaranSelanjutnya extends Component {
  constructor(props) {
    super(props);
    this.state = {
      category: [],
  ...
  async fetchApi() {
    ...
    await Axios.get(`${BASE_URL}/order/user/${getUser.userDetails.id}`).then(
      (response) => {
        this.setState({
          ...
          razor_id: response.data.order[0].order_razor,
          loading: false,
        });
      },
    );
    ...
    const response = await Axios.get(
      `https://api.razorpay.com/v1/orders/${this.state.razor_id}/payments`,
      {
  ...
  render() {
    ...
    const dailyData = this.state.category.filter((reading) =>
      reading.status_Order.includes('Menunggu Pembayaran Selanjutnya'),
    );
